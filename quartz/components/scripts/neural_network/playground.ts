...
function scrollTween(offset: number | { valueOf(): number; }) {
  return function() {
...
let heatMap =
    new HeatMap(300, DENSITY, xDomain, xDomain, d3.select("#heatmap"),
        {showAxes: true});
let linkWidthScale = d3.scaleLinear([0, 5],[1, 10]).clamp(true);
let colorScale = d3.scaleLinear<string, string>([-1, 0, 1],["#f59322", "#e8eaeb", "#0877bd"]).clamp(true);
let iter = 0;
...
function makeGUI() {
  ...
  dataThumbnails.on("click", function() {
    let newDataset = datasets[(this as HTMLElement).dataset.dataset!];
  ...
  regDataThumbnails.on("click", function() {
    let newDataset = regDatasets[(this as HTMLElement).dataset.regdataset!];
  ...
  let showTestData = d3.select<HTMLInputElement, any>("#show-test-data").on("change", function() {
  ...
  let discretize = d3.select<HTMLInputElement, any>("#discretize").on("change", function() {
  ...
  let percTrain = d3.select<HTMLInputElement, any>("#percTrainData").on("input", function() {
    state.percTrainData = parseInt(this.value, 10);
  ...
  let noise = d3.select<HTMLInputElement, any>("#noise").on("input", function() {
  ...
  let batchSize = d3.select<HTMLInputElement, any>("#batchSize").on("input", function() {
    state.batchSize = parseInt(this.value, 10);
  ...
  let activationDropdown = d3.select<HTMLInputElement, any>("#activations").on("change", function() {
  ...
  let learningRate = d3.select<HTMLInputElement, any>("#learningRate").on("change", function() {
  ...
  let regularDropdown = d3.select<HTMLInputElement, any>("#regularizations").on("change",
      function() {
  ...
  let regularRate = d3.select<HTMLInputElement, any>("#regularRate").on("change", function() {
  ...
  let problem = d3.select<HTMLInputElement, any>("#problem").on("change", function() {
    state.problem = problems[this.value as keyof typeof problems];
    generateData();
  ...
  let x = d3.scaleLinear([-1, 1],[0, 144]);
  let xAxis = d3
    .axisBottom(x)
    .tickValues([-1, 0, 1])
...
function updateWeightsUI(network: nn.Node[][], container: d3.Selection<d3.BaseType, unknown, HTMLElement, any>) {
...
function drawNode(cx: number, cy: number, nodeId: string, isInput: boolean,
    container: d3.Selection<SVGGElement, unknown, HTMLElement, any>, node?: nn.Node) {
  ...
  if (!isInput) {
    ...
    nodeGroup
      ...
      .on("mouseenter", function() {
        updateHoverCard(HoverType.BIAS, node, d3.pointer(container.node()));
...
function drawNetwork(network: nn.Node[][]): void {
  ...
  let layerScale = d3.scalePoint<number>(d3.range(1,numLayers - 1),[featureWidth, width - RECT_SIZE]).padding(0.7);
...
function getRelativeHeight(selection: d3.Selection<d3.BaseType, unknown, HTMLElement, any>) {
...
function updateHoverCard(type: HoverType | null, nodeOrLink?: nn.Node | nn.Link,
  ...
  d3.select("#svg").on("click", () => {
    ...
    let input = hovercard.select<HTMLInputElement>("input");
...
function drawLink(
    ...
    network: nn.Node[][], container: d3.Selection<SVGGElement, unknown, HTMLElement, any>,
  ...
  let diagonal = d3.linkHorizontal<any, { x: number; y: number }>().x((d) => d.y).y((d) => d.x)
  ...
  container.append("path")
    ...
    .on("mouseenter", function() {
      updateHoverCard(HoverType.WEIGHT, input, d3.pointer(container.node()));
    }).on("mouseleave", function() {
      updateHoverCard(null);
    });
  return line;
...
function updateUI(firstStep = false) {
  ...
  d3.select("#network").selectAll("div.canvas")
      .each(function(data) {
        const heatMapData = data as {heatmap: HeatMap, id: string};
        heatMapData.heatmap.updateBackground(reduceMatrix(boundary[heatMapData.id], 10),state.discretize);
  });
...
import { html } from "d3-fetch"; // Import html function from d3-fetch module
...
function initTutorial() {
  ...
  html(`tutorials/${state.tutorial}.html`).then((htmlFragment) => {
    tutorial.node()!.appendChild(htmlFragment.documentElement);
...
function drawDatasetThumbnails() {
  function renderThumbnail(canvas: any, dataGenerator: DataGenerator) {
    let w = 100;
    let h = 100;
    ...
    data.forEach(function(d: { label: d3.NumberValue; x: number; y: number; }) {
