...
export function classifyTwoGaussData(numSamples: number, noise: number):
  ...
  let varianceScale = d3.scaleLinear([0, .5],[0.5, 4]);
...
export function regressPlane(numSamples: number, noise: number):
  Example2D[] {
  let radius = 6;
  let labelScale = d3.scaleLinear([-10, 10],[-1, 1]);
  let getLabel = (x: number, y: number) => labelScale(x + y);
...
export function regressGaussian(numSamples: number, noise: number):
  ...
  let labelScale = d3.scaleLinear([0,2],[1,0]).clamp(true);
