...
export class HeatMap {
  ...
  constructor(
    ...
    this.xScale = d3.scaleLinear(xDomain,[0,width - 2* padding]);

    this.yScale = d3.scaleLinear(yDomain,[height - 2 * padding]);

    // Get a range of colors.
    let tmpScale = d3.scaleLinear<string, string>([0, .5, 1],["#f59322", "#e8eaeb", "#0877bd"]).clamp(true);
    ...
    this.color = d3.scaleQuantize([-1, 1],colors);
  ...
  private updateCircles(container: { selectAll: (arg0: string) => { (): any; new(): any; data: { (arg0: Example2D[]): any; new(): any; }; }; }, points: Example2D[]) {
    ...
    selection
      ...
      .style("fill", (d: { label: d3.NumberValue; }) => this.color(d.label));
